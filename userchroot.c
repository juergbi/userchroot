...
int main(int argc, char* argv[], char* envp[]) {
  ...
  if (argc >= 3 &&
    ...
    // this mode can only be run by users with write access to the
    // /dev directory in the chroot image.
    if (access(dev_path, R_OK | W_OK | X_OK) != 0) {
      fprintf(stderr,"install or uninstall devices can only be called with "
        "write access to /dev in the chroot. Error: %s\n", strerror(errno));
      exit(ERR_EXIT_CODE);
    }
